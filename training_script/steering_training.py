...
X_data, Y_data = load_multi_dataset(os.path.join(HDF5_PATH_DS3, f'train_ds2_h5_list.txt'))
...
#Prepare for convolutional operation
input_layer = Input(shape = NOTIMESTEPS_INPUT_SHAPE)
...
#Dense layers
dense_layer_1 = Dense(DENSE_HIDDEN_UNITS_1, activation=DENSE_ACTIVATION_FN) (flatten_layer)
#dropout_layer_2 = Dropout(0.5) (dense_layer_1) #commented out dropout layer as it contributed little to performance here.
...
#Model definitions
model = Model(inputs=input_layer, outputs=output_steering , name='evaluate_ds2_1')

model.summary()
plot_model(model, to_file=PLOT_MODEL_SAVE_FILE, show_shapes=PLOT_MODEL_SHOW_SHAPES)
model.compile(optimizer=Adam(lr=MODEL_LEARNING_RATE, decay = MODEL_LEARNING_DECAY), loss={"st": MODEL_LOSS_FN1})
