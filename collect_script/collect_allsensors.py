...
#image dimension of images stored in hard disk
IMAGE_DIM = (480,270)
...
class Collect(Node):
	def __init__(self):
		...
		self.get_logger().info(f'[{self.get_name()}] Initializing the collector node...')
                ...
                #creates two directories to store the contents from the collect process
		mkdir_p(CSV_PATH)
		mkdir_p(IMG_PATH)

                #ROS2 time synced subscriber nodes.
	...
	def callback(self, centre_camera, depth_centre, seg_centre, left90_camera,
		...
		self.get_logger().info(f"[{ts_sec}.{ts_nsec}] Format:{centre_camera.format}, Braking:{brake_cmd}")
                ...
                #reading and processing radar sensor data
		...
		#Calling functions to save images and non-image data
	...
	def save_image(self, centre_camera, depth_centre, seg_centre, left90_camera,
			depth_left, right90_camera, depth_right, msg_id):
                ...
                #Loading the image data, manipulating it, and storing into hard disk
