...
class Collect(Node):
	...
	def callback(self, centre_camera, depth_centre, control):
		...
		# else:    
		cmd_list = [linear_accel, throttle, steer_cmd, brake_cmd, velocity]
		
		#self.get_logger().info(f"saving images...")
		self.save_image(centre_camera, depth_centre, msg_id)
		self.save_csv(msg_id, cmd_list)
		
	def save_image(self, centre_camera, depth_centre, msg_id):
